		 	    	...
		 	    	<div class="card card-primary">
		 	    	    <div class="card-body">
	 	                  	<div class="form-group{{ $errors->has('company_id') ? ' has-error' : '' }}">
	 	                      	<select data-select="select" data-onchange=".role_id" data-valuename="company_id" data-ajaxurl="##" class="form-control input_required company_change" name="company_id">
	 	                         	<option value="">{{__('-- Select Company --')}}</option>
	 	                         	@if(isset($companies) && count($companies) > 0)
		 	                          	@foreach($companies as $company) 
		 	                            	<option value="{{ $company->id_companies ?? ''}}" {{old('company_id') == $company->id_companies ? 'selected' : ''}}>{{ $company->name ?? ''}}</option>
		 	                          	@endforeach
	 	                          	@endif
	 	                      	</select>
